...
const baseURL='http://localhost:8080/ProjectsManagement-Back-end/TaskRest';
...
const deleteTask = async (projectCode,taskCode) => {
    try {
      const response = await axiosInstance.delete(`/delete/${projectCode}/${taskCode}`);
...
const updateTask = async (codeTask, newStartDay, newEndDay, newDescription) => {
    try {
      const response = await axiosInstance.patch(`/update/${codeTask}/${newStartDay}/${newEndDay}/${newDescription}`);
      console.log('Response:', response);
      if (response.status === 200) {
